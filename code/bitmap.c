...
int main() {
	struct Bitmap bitmap = init_bitmap(45, 30);
	fill(bitmap, 0, 0, 45, 6, 0x58CEFA);
	fill(bitmap, 0, 6, 45, 12, 0xF5A9B8);
	fill(bitmap, 0, 12, 46, 18, 0xFFFFFF);
	fill(bitmap, 0, 18, 45, 24, 0xF5A9B8);
	fill(bitmap, 0, 24, 45, 30, 0x58CEFA);
	write_bitmap(bitmap, "output/out.bmp");
	free_bitmap(bitmap);
	return 0;
}